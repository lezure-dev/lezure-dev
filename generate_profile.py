...
def build_svg(theme="dark"):
    ...
    char_w = ascii_font * 0.615
    ascii_w = max_cols * char_w
    ascii_x = max(18, (divider_x - ascii_w) / 2)
    ...
    parts = []
    parts.append(f'<text x="{x}" y="28" class="header">{escape(PROFILE["display_name"])}@github</text>')
    parts.append(f'<line x1="{x+118}" y1="23" x2="{W-26}" y2="23" stroke="{border}" />')
    parts.append(f'<text x="{W-28}" y="42" text-anchor="end" class="sub">{escape(PROFILE["location_line"])}</text>')
    ...
    def add_section(title, y):
        parts.append(f'<text x="{x}" y="{y:.1f}" class="section">— {escape(title)} {"—"*25}</text>')
        return y + 15.5
...
.sub {{ fill: {muted}; font-size: 9.7px; }}
.key {{ fill: {accent}; font-size: {body_fs}px; }}
.value {{ fill: {text}; font-size: {body_fs}px; }}
.dots {{ fill: {dim}; font-size: {body_fs}px; letter-spacing: 0.1px; }}
.section {{ fill: {muted}; font-size: 9.5px; }}
</style>
<rect width="{W}" height="{H}" rx="16" fill="{bg}" stroke="{border}" />
<line x1="{divider_x}" y1="17" x2="{divider_x}" y2="{H-17}" stroke="{border}" />
